...
Vector3 Mirror::BRDF(const Vector3& in, const Vector3& normal, const Vector3& out, const bool& isFront) const {
    if (dot((in + out).normalize(), normal) > 0.99999) return m_ks / dot(normal, out);
    else return 0;
}
